...
/**
 * Get the currency unit text for the current language
 */
function getCurrencyUnit() {
    return translations[currentLang]?.currency_unit || 'сум';
}

/**
 * Format a price number with dot separators (e.g., 384.000)
 */
...
/**
 * Update all dynamic currency displays (policy price, coverage sum)
 * Called on language change and when pricing parameters change.
 */
function updateDynamicCurrencyDisplays() {
    const currency = getCurrencyUnit();
    const lang = currentLang;

    // Update policy price in summary panel
    const policyPriceEl = document.getElementById('policy_price');
    if (policyPriceEl) {
        ...
        policyPriceEl.textContent = `${formatPrice(price)} ${currency}`;
    }

    // Update coverage sum value
    const coverageSumValueEl = document.querySelector('[data-i18n="coverage_sum_value"]');
    if (coverageSumValueEl) {
        const sumText = translations[lang]?.coverage_sum_value || '80.000.000';
        coverageSumValueEl.textContent = `${sumText} ${currency}`;
    }
}
...
function updatePricingDisplay() {
    ...
    // Update the policy_price element in summary panel
    const policyPriceEl = document.getElementById('policy_price');
    if (policyPriceEl) {
        policyPriceEl.textContent = `${formatPrice(price)} ${currency}`;
    }

    // Update the cost-amount element (if it exists elsewhere)
    const costAmountEl = document.querySelector('.cost-amount');
    if (costAmountEl) {
        const usdPrice = (price / 12200).toFixed(2);
        costAmountEl.innerHTML = `${formatPrice(price)} ${currency} <span class="cost-usd">(${usdPrice} $)</span>`;
    }
}
...
const translations = {
    ru: {
        ...
        hero_subtitle: "Быстрое онлайн оформление страхового полиса",
        hero_desc: "Купите ОСАГО онлайн за 5 минут —<br>без очередей и бумаг.",
        hero_cta: "Купить ОСАГО Online",
        hero_btn_calculate: "Рассчитать",
        hero_trust_safe: "Безопасно",
        hero_trust_official: "Официально",
        ...
        // Wizard - main
        wizard_main_title: "Оформление полиса",
        wizard_step1_label: "Выбор ТС",
        wizard_step2_label: "Оформление",
        wizard_step3_label: "Оплата",

        // Wizard - Step 1: Vehicle data
        wizard_vehicle_title: "Данные авто",
        wizard_vehicle_subtitle: "Заполните данные",
        wizard_info_main: "Введите данные автомобиля, чтобы мы рассчитали предварительную стоимость ОСАГО для вашего региона.",
        wizard_info_secondary: "Данные используются только для расчёта и не передаются третьим лицам.",
        wizard_gov_number: "Гос. номер*",
        wizard_tech_passport: "Данные тех. паспорта*",
        wizard_load_btn: "Загрузить",
        wizard_model: "Модель ТС",
        wizard_type: "Тип ТС",
        wizard_region: "Регион регистрации",
        wizard_year: "Год выпуска",

        // Wizard - Personal data & policy terms
        wizard_personal_data: "Персональные данные",
        wizard_policy_terms: "Условия полиса",
        wizard_driver_count: "Количество водителей*",
        wizard_driver_unlimited: "Не ограничено",
        wizard_driver_limited: "Ограничено до 5 человек",
        wizard_start_date: "Начало страхования*",

        // Wizard - Step 2: Owner
        wizard_owner_title: "Информация о владельце",
        wizard_pinfl: "ПИНФЛ",
        wizard_owner_label: "Владелец:",
        wizard_passport_series: "Серия паспорта*",
        wizard_passport_number: "Номер паспорта*",
        wizard_phone: "Номер телефона*",
        wizard_phone_hint: "Номер телефона необходим для отправки электронного полиса",

        // Wizard - Step 3: Applicant
        wizard_applicant_title: "Информация о заявителе",
        wizard_applicant_is_owner: "Заявитель является владельцем",
        wizard_applicant_phone: "Телефон*",
        wizard_applicant_email: "Email",
        ...
        coverage_title: "Что покрывает полис",
        coverage_sum_label: "Страховая сумма",
        coverage_sum_value: "80.000.000",
        policy_price_label: "Стоимость полиса"
    },
    uz: {
        // Currency
        currency_unit: "so'm",
        ...
        hero_subtitle: "Tez onlayn sug'urta polisini rasmiylashtirish",
        hero_desc: "OSAGO ni 5 daqiqada onlayn xarid qiling —<br>navbat va qog'ozlarsiz.",
        hero_cta: "OSAGO sotib olish (Online)",
        hero_btn_calculate: "Hisoblash",
        hero_trust_safe: "Xavfsiz",
        hero_trust_official: "Rasmiy",
        ...
        // Wizard - main
        wizard_main_title: "Polisni rasmiylashtirish",
        wizard_step1_label: "TS tanlash",
        wizard_step2_label: "Rasmiylashtirish",
        wizard_step3_label: "To'lov",

        // Wizard - Step 1: Vehicle data
        wizard_vehicle_title: "Avto ma'lumotlari",
        wizard_vehicle_subtitle: "Ma'lumotlarni kiriting",
        wizard_info_main: "Avtomobil ma'lumotlarini kiriting, biz sizning hududingiz uchun OSAGO ning dastlabki narxini hisoblaymiz.",
        wizard_info_secondary: "Ma'lumotlar faqat hisoblash uchun ishlatiladi va uchinchi shaxslarga uzatilmaydi.",
        wizard_gov_number: "Davlat raqami*",
        wizard_tech_passport: "Texnik pasport ma'lumotlari*",
        wizard_load_btn: "Yuklash",
        wizard_model: "TS modeli",
        wizard_type: "TS turi",
        wizard_region: "Ro'yxatga olish hududi",
        wizard_year: "Ishlab chiqarilgan yili",

        // Wizard - Personal data & policy terms
        wizard_personal_data: "Shaxsiy ma'lumotlar",
        wizard_policy_terms: "Polis shartlari",
        wizard_driver_count: "Haydovchilar soni*",
        wizard_driver_unlimited: "Cheklanmagan",
        wizard_driver_limited: "5 kishigacha cheklangan",
        wizard_start_date: "Sug'urta boshlanishi*",

        // Wizard - Step 2: Owner
        wizard_owner_title: "Egasi haqida ma'lumot",
        wizard_pinfl: "JSHSHIR",
        wizard_owner_label: "Egasi:",
        wizard_passport_series: "Pasport seriyasi*",
        wizard_passport_number: "Pasport raqami*",
        wizard_phone: "Telefon raqami*",
        wizard_phone_hint: "Telefon raqami elektron polisni yuborish uchun kerak",

        // Wizard - Step 3: Applicant
        wizard_applicant_title: "Ariza beruvchi haqida ma'lumot",
        wizard_applicant_is_owner: "Ariza beruvchi egasi hisoblanadi",
        wizard_applicant_phone: "Telefon*",
        wizard_applicant_email: "Email",
        ...
        // Coverage / Summary panel
        coverage_title: "Polis nimani qoplaydi",
        coverage_sum_label: "Sug'urta summasi",
        coverage_sum_value: "80.000.000",
        policy_price_label: "Polis narxi"
    },
    en: {
        // Currency
        currency_unit: "sum",
        ...
        hero_subtitle: "Fast online insurance policy registration",
        hero_desc: "Buy OSAGO online in 5 minutes —<br>no queues, no paperwork.",
        hero_cta: "Buy OSAGO Online",
        hero_btn_calculate: "Calculate",
        hero_trust_safe: "Secure",
        hero_trust_official: "Official",
        ...
        // Wizard - main
        wizard_main_title: "Policy registration",
        wizard_step1_label: "Vehicle selection",
        wizard_step2_label: "Registration",
        wizard_step3_label: "Payment",

        // Wizard - Step 1: Vehicle data
        wizard_vehicle_title: "Vehicle data",
        wizard_vehicle_subtitle: "Fill in the data",
        wizard_info_main: "Enter your vehicle data so we can calculate the preliminary OSAGO cost for your region.",
        wizard_info_secondary: "Data is used only for calculation and is not shared with third parties.",
        wizard_gov_number: "Gov. number*",
        wizard_tech_passport: "Tech passport data*",
        wizard_load_btn: "Load",
        wizard_model: "Vehicle model",
        wizard_type: "Vehicle type",
        wizard_region: "Registration region",
        wizard_year: "Year of manufacture",

        // Wizard - Personal data & policy terms
        wizard_personal_data: "Personal data",
        wizard_policy_terms: "Policy terms",
        wizard_driver_count: "Number of drivers*",
        wizard_driver_unlimited: "Unlimited",
        wizard_driver_limited: "Limited to 5 people",
        wizard_start_date: "Insurance start date*",

        // Wizard - Step 2: Owner
        wizard_owner_title: "Owner information",
        wizard_pinfl: "PINFL",
        wizard_owner_label: "Owner:",
        wizard_passport_series: "Passport series*",
        wizard_passport_number: "Passport number*",
        wizard_phone: "Phone number*",
        wizard_phone_hint: "Phone number is required to send the electronic policy",

        // Wizard - Step 3: Applicant
        wizard_applicant_title: "Applicant information",
        wizard_applicant_is_owner: "Applicant is the owner",
        wizard_applicant_phone: "Phone*",
        wizard_applicant_email: "Email",
        ...
        // Coverage / Summary panel
        coverage_title: "What the policy covers",
        coverage_sum_label: "Insurance sum",
        coverage_sum_value: "80,000,000",
        policy_price_label: "Policy price"
    }
};
...
function setLanguage(lang) {
    ...
    // Update select option text with data-i18n
    ...
    // Refresh all dynamic currency displays
    updateDynamicCurrencyDisplays();
}
...
/**
 * Navigate to the main homepage from any state.
 * - Hides wizard container if active
 * - Shows main landing section
 * - Resets wizard state
 * - Scrolls to top smoothly
 * Works from any page: if on a different HTML page (e.g. /account),
 * the logo href="/" handles the redirect; this function handles SPA state.
 */
function goToHomepage() {
    const mainLanding = document.getElementById('main-landing');
    const wizardContainer = document.getElementById('wizard-container');

    // If wizard is visible, close it and reset
    if (wizardContainer && wizardContainer.style.display !== 'none') {
        wizardContainer.style.display = 'none';
        hideProgressIndicator();
    }

    // Show main landing content
    if (mainLanding) {
